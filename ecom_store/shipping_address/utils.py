...
def get_destination_id(serializer_data):
    ...
    params = {
        "keyword": serializer_data["subdistrict"].zip_code
    }
    ...
    matches = [
        ...
        if (
            ...
            and
            item["district_name"].upper()
            == serializer_data["district"].name.upper()
            and
            item["city_name"].upper()
            == serializer_data["city"].name.upper()
            and
            item["zip_code"]
            == serializer_data["subdistrict"].zip_code
        )
    ]
    ...
    if len(matches) > 1:
        raise serializers.ValidationError(
            {
                "destination": (
                    "Ditemukan lebih dari satu destination yang cocok."
                )
            }
        )
